...
export default function page() {
    return (
        <main className="py-4 mt-4 last:pr-5 snap-x snap-mandatory items-start flex space-x-4 overflow-x-scroll no-scrollbar pb-8">
            {cardData.map((item,index) => <RestaurantItemCard key={index} price={item.price} time={item.time} name={item.name} photoUrl={item.photoUrl} isVeg={item.isVeg} tags={item.tags} />)}
        </main>
    )
}
