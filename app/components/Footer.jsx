      ...
      <ul className="flex gap-2 items-center justify-center h-full">
        {navLinks.map((item, index) => (
          <Link
            key={index}
            href={item.pathName}
            className="flex-1  border-primary gap-2 flex flex-col items-center justify-center"
          >
            <Image
              src={pathName == item.pathName ? item.activeIcon : item.icon}
              width={20}
              height={20}
            />
            <p
              className={`capitalize text-yash  font-raleway ${
