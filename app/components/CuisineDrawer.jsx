...
export default function RestaurantDrawer() {
  ...
  return (
    ...
    >
      <div className=" bg-white py-1 mx-1 rounded-lg space-y-5 pb-8">
        <Image
          ...
          >
            {cuisine?.categories?.map((item, index) => (
              <Tab
                ...
                value={index}
                key={index}
                className={` text-sm font-raleway font-bold  ${
                  index == activeIndex ? "text-primary" : null
                }`}
              >
                {item?.name}
              </Tab>
          ...
          <div className="bg-white mt-2 rounded-lg overflow-hidden">
            ...
            <TabsBody>
              {cuisine?.categories?.map((category, index) => (
                <TabPanel value={index} key={index} className="px-2 divide-y-2">
                    ...
                    >
                      <div className="font-lato font-normal">
                        <Checkbox
                          size={20}
                          color="green"
                          className="w-5 h-5 font-semibold rounded-full aspect-square"
                          label={
                            <p className="font-lato font-semibold text-base text-[#444444]">
                              {item.name}
                            </p>
                          }
                        />
      ...
      <div className="bg-white w-full fixed flex justify-between flex-1 items-center shadow-md shadow-black bottom-0 z-50">
        <div className="flex items-center  rounded-lg gap-1 px-2 py-1 space-x-2 ml-3 shadow-xl shadow-gray-300 border-[1px] border-gray-500">
          ...
          <p className="font-lato font-normal text-base text-primary">30</p>
