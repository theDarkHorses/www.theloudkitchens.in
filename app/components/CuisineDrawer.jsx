...
export default function CuisineDrawer() {
  ...
  return (
    ...
    <div
      ...
      className={`flex  top-0 items-center justify-center fixed bottom-0 left-0 right-0 z-50 h-screen  transition-all backdrop-blur-sm no-scrollbar ${openDrawer ? " translate-y-0" : " translate-y-full"}`}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        style={{ height: windowHeight * 0.8 }}
        className={`rounded-t-lg overflow-hidden absolute left-0 right-0 bottom-0 bg-[#F6F6F6] overflow-y-scroll no-scrollbar  transition-all z-50 pb-20 ${openDrawer ? " translate-y-0" : " translate-y-full"}`}
      >
        ...
        <div className=" py-4 w-full mt-2 items-center no-scrollbar overflow-hidden overflow-y-scroll relative">
          <div className="w-full flex-1 mx-2 py-2  flex justify-start text-sm items-center space-x-2">
            {cuisine?.categories?.map((item, index) => (
              <div key={index} onClick={() => setActiveIndex(index)} className={`cursor-pointer font-semibold font-raleway border-primary ${activeIndex == index ? "border-b-2 text-primary" : ""}`}>{item.name}</div>
            ))}
          </div>
        </div>
            ...
            {cuisine?.categories[activeIndex]?.items?.map((item, index) => (
              <div className="flex justify-between items-center   space-x-2" key={index}>
                <div className="font-lato font-normal">
                  <div className="flex items-center space-x-2">
                    <input id={"checkbox" + index} type="checkbox" size={20} color="green" className="w-5 accent-green-600 h-5 font-semibold rounded-full aspect-square" />
                    <label htmlFor={"checkbox" + index} className="font-lato font-semibold text-sm text-[#444444]">{item?.name}</label>
                  </div>
                </div>
                <div className="bg-[#ac232320] text-sm flex items-center  rounded-lg gap-1 px-2 py-1 space-x-2">
                  ...
                  <p className="font-lato font-normal text-sm text-primary">1</p>
        ...
        <div className="bg-white w-full shadow-black fixed left-0 right-0 flex justify-end flex-1  shadow-md  bottom-0 z-50">
