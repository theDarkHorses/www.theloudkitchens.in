...
export default function page() {
    ...
    useEffect(() => {
        async function getAddress(addressId) {
            const addressRef = doc(collection(doc(userCollectionRef, userId), "addresses"), addressId);
            const address = await getDoc(addressRef);
            return address.data();
        }

        getAddress(params.id).then((data) => {
            setAddress({ id: params.id, address: data.address || "", birthday: data.birthday || "", landmark: data.landmark || "", name: data.name || "" })
            setOrderFor(data?.orderFor || orderForOptions[0])
            setLocation(data?.location || locationOptions[0])
        })
    }, []
    )
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            await setDoc(doc(collection(doc(userCollectionRef, userId), "addresses"), params.id), { ...address, orderFor, location })
        } catch (err) {
